...
const Hero = ({ lang = "en" }) => {
  return (
    ...
    >
      ...
      <div className="flex-grow flex flex-col justify-center gap-6 sm:gap-8 lg:gap-10 max-w-5xl mx-auto w-full">
        <div className="flex flex-col gap-4 sm:gap-6 w-full">
          <h1 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-bold tracking-tight text-slate-900 flex items-center gap-3">
            Hi! <span className="animate-wave origin-[70%_70%] inline-block">👋</span>
          </h1>
          <div className="flex flex-row gap-2 sm:gap-4 justify-start items-center overflow-visible flex-wrap">
            ...
            <div className="relative">
              <img
                ...
                alt="Apif Supriadi"
                className="w-16 h-16 sm:w-20 sm:h-20 md:w-24 md:h-24 lg:w-28 lg:h-28 rounded-full border-2 sm:border-4 border-newsky flex-shrink-0 object-cover"
                loading="eager"
              />
            </div>
        ...
        {lang === "en" ? (
          ...
          <div className="flex flex-col max-w-2xl lg:max-w-3xl">
            <p className="text-base sm:text-lg md:text-xl lg:text-2xl leading-relaxed text-slate-700 font-medium">
              Saya adalah seorang Web Developer yang beralih menjadi    <span className="text-newblue font-bold">Mobile App Developer</span>. Saya menikmati bekerja di ekosistem JavaScript, mengeksplorasi teknologi baru, dan mempelajari hal baru. Selain itu, saya adalah penggemar sepak bola <span>&#9917;</span>.
            </p>
          </div>
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-6 mt-2">
          <div className="w-full sm:w-auto">
            <a
              href="#blog"
              className="inline-flex items-center justify-center px-8 py-3.5 bg-newblue text-white font-bold rounded-xl shadow-lg shadow-newblue/20 hover:bg-blue-600 hover:-translate-y-0.5 transition-all duration-300 w-full sm:w-auto"
            >
          ...
          <div className="flex flex-row gap-4">
            {SOCIAL_LINKS.map((link, index) => (
              <a
                ...
                className="p-3.5 rounded-full bg-slate-50 text-slate-600 hover:text-newblue hover:bg-newsky/10 transition-all duration-300 border border-slate-100 shadow-sm"
      ...
      <div className="w-full mt-auto py-6 sm:py-8 border-t border-slate-50 bg-white/80 backdrop-blur-sm">
        <div className="overflow-hidden relative flex items-center">
          <div className="flex animate-marquee whitespace-nowrap gap-12 sm:gap-20 h-full items-center">
            ...
            {[...SKILLS.flatMap(cat => cat.lists), ...SKILLS.flatMap(cat => cat.lists)].map((skill, idx) => (
